...
// The SDK retries 408/409/429/5xx and connection errors — 429/408/409 are
// treated as transient by design (rate limit, request timeout, conflict),
// never a genuine client mistake like 400/401/404 — so this needs no
...
export class AnthropicClientService {
  ...
  // Belt-and-suspenders for the "key never appears in logs" AC: the SDK
  // sends it as a header and never echoes it into error bodies, but a
  // literal-substring redaction costs nothing and doesn't rely on that.
