...
describe('cursor encode/decode', () => {
  it('round-trips a cursor losslessly', () => {
    const cursor: Cursor = { updatedAt: '2024-03-14T09:26:53.589793Z', id: 'a1b2c3d4-0000-0000-0000-000000000001' };
  ...
  it('preserves microsecond precision on the timestamp', () => {
    ...
    const cursor: Cursor = { updatedAt: '2024-03-14T09:26:53.123456Z', id: 'a1b2c3d4-0000-0000-0000-000000000002' };
    const decoded = decodeCursor(encodeCursor(cursor));
    expect(decoded.updatedAt).toBe('2024-03-14T09:26:53.123456Z');
  });

  it('produces a URL-safe token (base64url, no padding/plus/slash)', () => {
    const token = encodeCursor({ updatedAt: '2024-03-14T09:26:53.123456Z', id: 'a1b2c3d4-0000-0000-0000-000000000003' });
  ...
  it('rejects a truncated cursor (decodes to incomplete JSON)', () => {
    const token = encodeCursor({ updatedAt: '2024-03-14T09:26:53.123456Z', id: 'a1b2c3d4-0000-0000-0000-000000000004' });
  ...
  it('rejects a tampered cursor (valid base64url/JSON, wrong shape)', () => {
    expect(() => decodeCursor(rawCursor({ updatedAt: '2024-03-14T09:26:53.123456Z' }))).toThrow(InvalidCursorError);
  });

  it('rejects a cursor whose updatedAt is not a valid timestamp', () => {
    expect(() => decodeCursor(rawCursor({ updatedAt: 'not-a-date', id: 'x' }))).toThrow(InvalidCursorError);
  });
  ...
  it('rejects extra fields (tampered payload)', () => {
    expect(() =>
      decodeCursor(rawCursor({ updatedAt: '2024-03-14T09:26:53.123456Z', id: 'x', role: 'ADMIN' })),
  ...
  it('property: decode(encode(x)) === x across generated inputs', () => {
    for (let i = 0; i < 200; i++) {
      const cursor: Cursor = {
        updatedAt: new Date(Date.now() - Math.floor(Math.random() * 1e12)).toISOString().replace(
          /\.\d+Z$/,
