...
| Status filter and a moderator rejected-review view | A contributor already sees their rejected listings in "My listings" (all statuses, status-badged); moderators only get the pending queue, and neither has a rejected-only filter. Reviewing past rejections is an operational nicety the core submit→moderate→publish loop doesn't need | Add `status` to `CatalogQuery` and to the repository visibility rules — a contributor may filter only within their own listings, a moderator across all — then a status `Select` in the filter bar and a moderator entry point. The visibility half touches the same status-scoping that was handled as a security PR |
